(function () {
  ...
  function install() {
    ...
    style.textContent = `
      ...
      .finance-cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.finance-card{position:relative;padding:14px;border:1px solid var(--line);border-radius:11px;background:linear-gradient(145deg,var(--surface-2),var(--surface-end));overflow:hidden}.finance-card::before{content:"";position:absolute;inset:0 auto 0 0;width:3px;background:#4b7be5}.finance-card:nth-child(1)::before{background:#35b37e}.finance-card:nth-child(2)::before{background:#e9b949}.finance-card:nth-child(3)::before{background:#ff7885}.finance-card:nth-child(4)::before{background:#9b7bea}
